...
namespace YellowstonePathology.Business.Gateway
{
	...
	{               
		...
		public static Flow.FlowLogList GetFlowLogListByReportNo(string reportNo)
		{
			...
			SqlCommand cmd = new SqlCommand("select pso.ReportNo, ao.PLastName, ao.PFirstName, ao.AccessionDate, pso.FinalDate, pso.PanelSetName [TestName], pso.ObjectId " +
				"from tblPanelSetOrder pso join tblAccessionOrder ao on pso.MasterAccessionNo = ao.MasterAccessionNo " +
                "where pso.PanelSetId not in (19,143) and pso.CaseType = 'Flow Cytometry' and pso.ReportNo = @ReportNo ");
			cmd.CommandType = CommandType.Text;
		...
		public static Flow.FlowLogList GetByAccessionMonth(DateTime date)
		{
			...
			SqlCommand cmd = new SqlCommand("Select pso.ReportNo, ao.PLastName, ao.PFirstName, ao.AccessionDate, pso.FinalDate, pso.PanelSetName [TestName], pso.ObjectId " +
                "from tblPanelSetOrder pso join tblAccessionOrder ao on pso.MasterAccessionNo = ao.MasterAccessionNo " +
				"where pso.PanelSetId not in (19,143) and pso.CaseType = 'Flow Cytometry' and month(ao.AccessionDate) = @Month and Year(ao.AccessionDate) = @Year " +
		...
		public static Flow.FlowLogList GetByPatientName(string patientName)
		{
			...
			string sql = "select pso.ReportNo, ao.PLastName, ao.PFirstName, ao.AccessionDate, pso.FinalDate, pso.PanelSetName [TestName], pso.ObjectId " +
                "from tblPanelSetOrder pso join tblAccessionOrder ao on pso.MasterAccessionNo = ao.MasterAccessionNo " +
                "Where pso.PanelSetId not in (19,143) and pso.CaseType = 'Flow Cytometry' ";
