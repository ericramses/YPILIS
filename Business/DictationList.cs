...
namespace YellowstonePathology.Business
{
    public class DictationList : FileList
    {
        ...
        private void LoadDirectory()
        {
            ...
            switch (this.m_DictationLocation)
            {
                case DictationLocationEnum.Local:
                    this.LoadDirectory(YellowstonePathology.Business.Properties.Settings.Default.LocalDictationFolder);
                    break;
                case DictationLocationEnum.Server:
                    foreach (DirectoryInfo d in new DirectoryInfo(YellowstonePathology.Business.Properties.Settings.Default.ServerDictationFolder).GetDirectories())
                    {
        ...
        public void MoveServerFileToLocal(FileListItem item)
        {
            ...
            string moveDestination = YellowstonePathology.Business.Properties.Settings.Default.LocalDictationFolder + slashSplit[slashSplit.Length - 1];
