...
namespace YellowstonePathology.Business.HL7View.CDC
{
	...
	{        		
        ...
        public MTDohPidView(string patientId, string lastName, string firstName, Nullable<DateTime> birthdate, 
        ...
        {                       
            ...
            string hl7PatientIdentifier = "^^^^MR^~*SSN*^^^^SS^~*PATIENTID*^^^^PI^Yellowstone Pathology&*CLIA*&CLIA";
            hl7PatientIdentifier = hl7PatientIdentifier.Replace("*SSN*", YellowstonePathology.Business.Helper.SsnHelper.FormatWithoutDashes(this.m_Ssn));
            hl7PatientIdentifier = hl7PatientIdentifier.Replace("*PATIENTID*", this.m_PatientId);
            hl7PatientIdentifier = hl7PatientIdentifier.Replace("*CLIA*", MTDohResultView.CLIANUMBER);

            XElement pid03Element = new XElement("PID.3");
            YellowstonePathology.Business.Helper.XmlDocumentHelper.AddElement("PID.3.1", hl7PatientIdentifier, pid03Element);
            YellowstonePathology.Business.Helper.XmlDocumentHelper.AddElementIfNotEmpty(pidElement, pid03Element);

            //|^^^^MR^~^^^^SS^~1234^^^^PI^PATHWAY&07D1035411&CLIA|
