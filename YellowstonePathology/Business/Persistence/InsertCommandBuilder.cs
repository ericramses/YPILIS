...
namespace YellowstonePathology.Business.Persistence
{
    ...
    {                        
        ...
        public void Build(object parentObject, Queue<MySqlCommand> insertCommands, Queue<MySqlCommand> insertLastCommands)
        {            
        ...
        public void BuildSubclassOnly(object parentObject, Queue<MySqlCommand> insertCommands, Queue<MySqlCommand> insertLastCommands)
        {            
        ...
        private void HandlePersistentChildCollections(object parentObject, Type objectType, Queue<MySqlCommand> insertCommands, Queue<MySqlCommand> insertLastCommands)
        {
        ...
        private void HandlePersistentChildren(object objectToInsert, Type objectType, Queue<MySqlCommand> insertCommands, Queue<MySqlCommand> insertLastCommands)
        {
        ...
        private void ProcessObjectForInsert(object objectToInsert, Type objectType, Queue<MySqlCommand> insertCommands, Queue<MySqlCommand> insertLastCommands)
        {                        
            ...
            if (this.m_InsertSubclassOnly == false)
            {
                if (persistentClassAttribute.HasPersistentBaseClass == true)
                {
                    ...
                    {
                        MySqlCommand sqlCommand = this.CreateSqlCommand(keyPropertyBridge, basePropertyBridgeList, persistentClassAttribute.BaseStorageName, keyAttribute.IsAutoGenerated);
            ...
            if (persistentClassAttribute.IsPersisted == true)
            {
                ...
                if (persistentClassAttribute.IsManyToManyRelationship == true)
                {
                    MySqlCommand sqlCommandToAdd = this.CreateSqlCommand(keyPropertyBridge, propertyBridgeList, persistentClassAttribute.StorageName, keyAttribute.IsAutoGenerated);
                    insertLastCommands.Enqueue(sqlCommandToAdd);
                }
                else
                {
                    MySqlCommand sqlCommand = this.CreateSqlCommand(keyPropertyBridge, propertyBridgeList, persistentClassAttribute.StorageName, keyAttribute.IsAutoGenerated);
                    insertCommands.Enqueue(sqlCommand);
                }
            }            
        }        

        private MySqlCommand CreateSqlCommand(PropertyBridge keyPropertyBridge, List<PropertyBridge> propertyBridgeList, string storageName, bool isAutoGenerated)
        {
